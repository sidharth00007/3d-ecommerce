        ...
        <div className="py-14 px-4 md:px-6">
            <div className="flex justify-start items-start space-y-2 flex-col">
                <h1 className="text-3xl lg:text-4xl font-bold leading-7 lg:leading-9 text-gray-900">
                    Order Number : {orderDetails && orderDetails._id && orderDetails._id.substring(0, 8)}
                </h1>
                <p className="text-base font-medium leadong-6 text-gray-600">
                    {orderDetails &&
                        orderDetails.createdAt &&
                        (() => {
                            const rawDate = orderDetails.createdAt.split("T")[0];
                    ...
                    {orderDetails &&
                        orderDetails.createdAt &&
                        (() => {
                            const rawTime = orderDetails.createdAt.split("T")[1].split(".")[0];
                            const timeArray = rawTime.split(":");
                            const hoursUTC = parseInt(timeArray[0], 10);
                            const minutes = parseInt(timeArray[1], 10);

                            // Convert to IST (UTC+5:30)
                            let hoursIST = (hoursUTC + 5);
                            const minutesIST = (minutes + 30) % 60; // Add 30 minutes
                            ...
                            hoursIST = hoursIST % 24;

                            const period = hoursIST >= 12 ? "PM" : "AM";
                            const formattedHours = hoursIST % 12 === 0 ? 12 : hoursIST % 12;

                            return `${formattedHours}:${minutesIST < 10 ? '0' : ''}${minutesIST} ${period}`;
                        })()
                    }
